...
const EXAMPLE_OPTIONS = [
    { name: "Simple Example", value: "simple", fn: createExampleFLF },
    {
        ...
        value: "minimal",
        fn: () => createMinimalFLF("L-DEMO-001"),
    },
...
function App() {
    ...
    const [exampleType, setExampleType] =
        useState<(typeof EXAMPLE_OPTIONS)[number]["value"]>("simple");
    const [initialManifest, setInitialManifest] = useState<
        FLFManifest | undefined
    >(() => {
        // 使用简单示例作为初始值
    ...
    const handleExampleChange = (
        type: (typeof EXAMPLE_OPTIONS)[number]["value"],
    ) => {
                    ...
                    <div className="example-buttons">
                        {EXAMPLE_OPTIONS.map((option) => (
                            <button
                                ...
                                onClick={() =>
                                    handleExampleChange(option.value)
                                }
                            >
                                {option.name}
                            </button>
